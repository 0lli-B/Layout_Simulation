...
L6 = [0, 0, 0, 0, 0, 0, 5000]
L7 = [0, 0, 0, 0, 0, 0, 0]
transport_matrix = np.array([L1, L2, L3, L4, L5, L6, L7], dtype='float32')
...
column_names = ['Raw_parts_x', 'Raw_parts_y', 'Machine_V_x', 'Machine_V_y',
                ...
                'Distance_Y_Z', 'Distance_Y_FL', 'Distance_Z_FL', 'Total_transport_way[units]',
...
class Layout:
    ...
    def formula_distance(self, x1, x2, y1, y2):  # d = ((x1-x2)²+(y1-y2)²)^0.5
        distance = np.sqrt(np.square(x1 - x2, dtype='float32') + np.square(y1 - y2, dtype='float32'), dtype='float32')
        return distance  # distance between two points

    def calculate_transport_way(self):  # calculate all relevant distances
        distance_RL_V = self.formula_distance(self.Raw_Parts.x, self.Machine_V.x, self.Raw_Parts.y, self.Machine_V.y)
        distance_RL_X = self.formula_distance(self.Raw_Parts.x, self.Machine_X.x, self.Raw_Parts.y, self.Machine_X.y)
        ...
        distance_W_FL = self.formula_distance(self.Machine_W.x, self.Finished_Parts.x, self.Machine_W.y, self.Finished_Parts.y)
        ...
        distance_Y_FL = self.formula_distance(self.Machine_Y.x, self.Finished_Parts.x, self.Machine_Y.y, self.Finished_Parts.y)
        distance_Z_FL = self.formula_distance(self.Machine_Z.x, self.Finished_Parts.x, self.Machine_Z.y, self.Finished_Parts.y)
        # create a matrix with the distances in the right spots
        matrix_distance = np.array([[0, distance_RL_V, 0, distance_RL_X, 0, 0, 0],
                                    [0, 0, distance_V_W, 0, 0, distance_V_Z, 0],
                                    [0, 0, 0, distance_W_X, 0, distance_W_Z, distance_W_FL],
                                    [0, 0, 0, 0, distance_X_Y, 0, 0],
                                    [0, 0, 0, 0, 0, distance_Y_Z, distance_Y_FL],
                                    [0, 0, 0, 0, 0, 0, distance_Z_FL],
        ...
        return distance_RL_V, distance_RL_X, distance_V_W, distance_V_Z, distance_W_X, distance_W_Z, distance_W_FL, distance_X_Y, distance_Y_Z, distance_Y_FL, distance_Z_FL, transport_route


list_RL_x = []  # lists for the results
list_RL_y = []
list_V_x = []
list_V_y = []
list_W_x = []
...
for simulation in range(NUMBER_OF_SIMULATIONS):  # run number of desired simulations
    ...
    # calculate distances
    distance_RL_V, distance_RL_X, distance_V_W, distance_V_Z, distance_W_X, distance_W_Z, distance_W_FL, distance_X_Y, distance_Y_Z, distance_Y_FL, distance_Z_FL, transport_route = layout.calculate_transport_way()
    list_RL_x.append(layout.Raw_Parts.x)  # append to lists
    list_RL_y.append(layout.Raw_Parts.y)
    ...
    list_FL_x.append(layout.Finished_Parts.y)
    list_FL_y.append(layout.Finished_Parts.y)
    list_distance_RL_V.append(distance_RL_V)
    list_distance_RL_X.append(distance_RL_X)
    ...
    list_distance_W_FL.append(distance_W_FL)
    list_distance_X_Y.append(distance_X_Y)
    list_distance_Y_Z.append(distance_Y_Z)
    list_distance_Y_FL.append(distance_Y_FL)
    list_distance_Z_FL.append(distance_Z_FL)
    list_transport_way_result.append(transport_route)
    list_transport_way_result_meter.append(transport_route * 5)


new_df = pd.DataFrame(columns=column_names)  # make new df from simulation results
new_df['Raw_parts_x'] = list_RL_x  # assign lists to correct column
new_df['Raw_parts_y'] = list_RL_y
...
new_df['Finished_parts_x'] = list_FL_x
new_df['Finished_parts_y'] = list_FL_y
new_df['Distance_RL_V'] = list_distance_RL_V
new_df['Distance_RL_X'] = list_distance_RL_X
...
new_df['Distance_W_FL'] = list_distance_W_FL
new_df['Distance_X_Y'] = list_distance_X_Y
new_df['Distance_Y_Z'] = list_distance_Y_Z
new_df['Distance_Y_FL'] = list_distance_Y_FL
new_df['Distance_Z_FL'] = list_distance_Z_FL
